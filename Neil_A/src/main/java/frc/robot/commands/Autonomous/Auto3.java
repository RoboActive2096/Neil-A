...
public class Auto3 extends SequentialCommandGroup {
  ...
  public Auto3(Shooter sh,DriveBase db,FourBar fb) {
    ...
    super(new GyroTurn(db, 11, false),new ShooterForSecond(sh,2,1.5),new GyroTurn(db, 150, false),new openfourBarWhileDriving(db, 4.9, fb),new GyroTurn(db, 159, true),new FourBarWheelsOn(fb),new FourBarClose(fb),new parallelShooterWithFourBar(sh,fb, 4),new FourBarWheelsOff(fb),new delayAuto(15));
  }
}
